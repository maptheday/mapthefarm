...
send("CRSFSTART:1")
time.sleep(0.2)
assert wait_for_status(phase="PARKED", gate="NONE"), "Expected START to be rejected with no GPS fix"
...
send("CRSFSTART:1")
time.sleep(0.2)
assert wait_for_status(phase="RAISE", gate="NONE"), "Expected START to be ignored mid-RAISE"
...
# ── Start mission ────────────────────────────────────────────────────────
send("CRSFSTART:1")
assert approve_gate("MISSION", reason="OPERATOR_START", timeout=5), "Mission did not start"

for alt in [20, 30]:
    set_world(alt_ft=float(alt))
    time.sleep(0.5)

assert wait_for_status(phase="MISSION", wp=1, timeout=10), "Never reached WP0"
...
send("CRSFSTOP:1")
assert approve_gate("PARKED", reason="EMERGENCY_STOP", timeout=5), "E-stop did not return to PARKED"
...
send("CRSFSTART:1")
assert approve_gate("RAISE", reason="OPERATOR_START", timeout=5), "Re-arm failed"
...
send("CRSFSTART:1")
assert approve_gate("MISSION", reason="OPERATOR_START", timeout=5), "Second mission did not start"

for alt in [20, 30]:
    set_world(alt_ft=float(alt))
    time.sleep(0.5)

assert wait_for_status(phase="MISSION", wp=1, timeout=10), "Never reached WP0 on second mission"
...
assert wait_for_status(phase="MISSION", wp=2, timeout=15), "Never reached WP1"
...
assert wait_for_status(phase="MISSION", wp=3, timeout=15), "Never reached WP2"
...
assert approve_gate("HOVER_SETTLE", reason="MISSION_COMPLETE", timeout=15), \
    "Mission did not enter hover settle"

# ── Hover settle (10s) ───────────────────────────────────────────────────
set_world(alt_ft=30.0)
assert approve_gate("LANDING", reason="HOVER_COMPLETE", timeout=15), \
    "Hover settle did not enter landing"
...
assert approve_gate("LANDED", reason="TOUCHDOWN", timeout=30), \
    "Never landed"
